...
int main(int argc, char **argv) {
    ...
    context.activate();
    {
        ...
        gpu::gpu_mem_32f res;
        res.resizeN(height * width);

        timer t;
        for (int i = 0; i < benchmarkingIters; ++i) {
            kernel.exec(gpu::WorkSize(16, 16, width, height), res, width, height, centralX - sizeX / 2.0f,
                        centralY - sizeY / 2.0f, sizeX, sizeY, iterationsLimit, false);
            t.nextLap();
        }

        res.readN(gpu_results.ptr(), height * width);
